...
The reference to the `level` variable in this code may seem confusing, because there is no variable called `level` in the `faithfuld` data. In this context the `after_stat(level)` notation refers to a variable computed internally (see Section \@ref(generated-variables)). To display the same density as a heat map, you can use `geom_raster()`:
